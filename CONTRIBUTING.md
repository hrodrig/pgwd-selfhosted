...
The install path **`helm repo add pgwd https://hrodrig.github.io/pgwd-selfhosted`** expects **`index.yaml`** and packaged **`.tgz`** files on the **`gh-pages`** branch. Static **`index.html`** and **`.nojekyll`** (disables Jekyll so Pages serves that HTML instead of the **`gh-pages`** `README.md` theme) are copied from **`run/kubernetes/helm/helm-repo-landing/`** on each **Release Charts** run — same idea as **gghstats-selfhosted**.
