...
[![Version](https://img.shields.io/badge/version-0.1.17-blue)](https://github.com/hrodrig/pgwd-selfhosted/releases)
...
**Multi-database YAML (`databases:`):** **multi-entry** **`databases:`** cannot be combined with **`-kube-postgres`** in the same process (a **single** entry + kube is supported); SQLite history uses **`(client, cluster, database)`** (not the URL host) — use a **unique `client` per entry** when the same DB name appears on different hosts. See **[Multi-database limitations](https://github.com/hrodrig/pgwd/blob/main/README.md#multi-database-limitations)** in the pgwd README.
...
Use the **`version:`** from **`helm search`** once **`index.yaml`** is live (it may differ from **`0.1.9`**). If **`helm repo add`** or search fails, the repo may not be published yet — use the clone path above.
...
You may use **`config.enabled: true`** for a full config file (`databases:` + notifications, etc.); see the chart **[README](run/kubernetes/helm/pgwd/README.md)**.
...
- **`Chart.yaml` → `appVersion`** — **pgwd** application / image line; align with [pgwd releases](https://github.com/hrodrig/pgwd/releases) when you bump the deployed image story (defaults in this repo track **v1.1.1**).
