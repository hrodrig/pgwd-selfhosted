...
Single **pgwd** service using the **GHCR** image. Default **`PGWD_VERSION=v0.7.0`**: env-driven Postgres monitoring and optional Slack/Loki; use **`docker logs`** for output. Optional **SQLite**, **`PGWD_HTTP_LISTEN`**, and published **ports** — see **[`docker-compose.yml`](docker-compose.yml)** comments and **[`run/common/.env.example`](../../common/.env.example)**.
