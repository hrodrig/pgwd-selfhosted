...
Run the published image without Compose. Examples use **[pgwd](https://github.com/hrodrig/pgwd) v1.0.0** on GHCR (**distroless**; no shell in the container): Postgres monitoring, optional Slack/Loki, and (when configured) SQLite history, HTTP **`/metrics`**, hysteresis — see **upstream** docs. Configure with **environment variables** (or a **mounted config file** and **`PGWD_CONFIG`**).
...
**Compose vs `docker run`:** different layouts may apply for other releases. For Compose with **[`run/common/.env.example`](../common/.env.example)**, use **[`run/docker-compose/minimal`](../docker-compose/minimal/README.md)**. This **`docker run`** section tracks **v1.0.0** as the default documented tag; pin another **`PGWD_VERSION`** if needed.
