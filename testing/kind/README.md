...
| **`make test-helm-kind`** | Same Postgres + **`helm upgrade --install`** pgwd (README-style **`PGWD_DRY_RUN`**) + rollout + **log check** for Postgres stats (chart default image tag, e.g. **v0.6.6**). |
